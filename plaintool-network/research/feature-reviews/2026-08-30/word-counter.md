...
- 결론: 현재 도구는 “작고 정확한 다국어 기본 카운터”로는 탄탄하다. 경쟁 도구가 제공하는 문장 수·읽기 시간·키워드 빈도까지 필요한 사용자를 놓치고 있으므로, 기본 화면을 복잡하게 만들지 않는 보조 지표 확장이 다음 개선점이다.
...
| P1 | `문장 수`와 `예상 읽기 시간`을 접힌 `Options/Details`에 추가 | 높음 | 높음 | 중간 | 두 경쟁군 모두 제공하며 작성 길이 확인의 다음 자연스러운 질문이다. 로케일별 문장 경계와 WPM 가정은 결과에 명시해야 한다. |
| P1 | 결과 근처에 계산 규칙 도움말 추가 | 중간 | 높음 | 낮음 | “문자=grapheme”, 문단=빈 줄 구분, fallback 시 근사값이라는 의미를 짧게 설명하면 숫자 신뢰성이 올라간다. |
| P2 | 상위 단어 빈도(기본 off)를 별도 Options 또는 장문 분석 경로로 추가 | 중간 | 중간 | 중간 | 수요는 관찰되지만 stopword와 CJK tokenization 정책이 필요하다. `keyword density`를 SEO 점수처럼 해석시키지 않아야 한다. |
