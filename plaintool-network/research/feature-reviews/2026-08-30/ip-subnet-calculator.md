...
| --- | --- | --- | --- | --- | --- |
| P1 | IPv6 전용 모드: prefix, 범위, address count, `/127`·`/128` 의미 | 경쟁 기능 격차 축소 | 높음 | 큼: 별도 core/계약/17 locale/QA | 후보 |
| P1 | 결과 행별 복사 버튼 | 네트워크 설정에 필요한 한 값만 빠르게 복사 | 높음 | 작음~중간 | 후보 |
| P2 | subnet split과 host-count→prefix | 실제 설계 작업 지원 | 높음 | 큼 | 후보 |
| P2 | range→최소 CIDR 집합 | ACL/라우팅 목록 작성 지원 | 중간 | 큼 | 후보 |
