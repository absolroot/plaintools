export type CssFormatterSnapshot = {
  revision: number;
  input: string;
  output: string;
  stale: boolean;
...
/** Feature-owned result authority; Worker cancellation is an additional guard. */
export class CssFormatterAuthority {
  #snapshot: CssFormatterSnapshot = {
    revision: 0,
    input: "",
    output: "",
    stale: false,
  ...
  get snapshot(): Readonly<CssFormatterSnapshot> {
