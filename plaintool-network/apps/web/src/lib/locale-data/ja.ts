...
const bundle: LocaleBundle = {
  ...
  preview: {
    ...
    json: {
      ...
      faqs: [
        ...
        {
          q: "Prettier で JSON を整形するのと似ていますか？",
          a: "どちらもインデントと改行で有効な JSON を読みやすくします。このツールはブラウザーで厳密な JSON を処理しますが、Prettier の実行、コメントの保持、オブジェクトキーの並べ替えを約束するものではありません。",
        },
      ],
    },
    time: {
  ...
  network: {
    allTools: "すべてのツール",
    directoryMetaTitle: "AbsolTools | 必要なとき、すぐ使えるツール",
    ...
    directoryIntro:
      "登録不要・サーバー保存なしで、すべてのツールを無料で使えます。次回すぐに開けるよう、このサイトをブックマークしてください。",
    toolPromise: {
      improvement: "AbsolToolsは、よく使うツールをより正確で使いやすくします。",
      privacy:
        "すべてのツールは無料で、登録も不要です。処理はブラウザー内で行われ、入力内容や結果がサーバーに保存・送信されることはありません。",
      bookmark: "次回すぐに使えるよう、このサイトをブックマークしてください。",
    },
