...
let localEvidenceAvailable = true;
try {
  await access(manifestRoot);
} catch (error) {
...
if (localEvidenceAvailable) {
  for (const [featureId, manifestFile] of featureManifests) {
    ...
    if (
      ...
      errors.push(
        `${featureId} copyDirectories must be an array when present.`,
      );
      continue;
    }
    ...
    try {
      ...
      errors.push(
        `${featureId} locale source cannot be read: ${error.message}`,
      );
    }
    ...
    for (const locale of locales) {
      ...
      if (
        !Array.isArray(review.referenceIds) ||
        review.referenceIds.length < 2
      ) {
        errors.push(
  ...
  console.warn(
    "Locale review evidence is unavailable in this checkout; skipping local-only evidence validation.",
  );
}
